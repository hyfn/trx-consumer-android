...
class LiveWorkoutTableAdapter(
    private val listener: LiveWorkoutTableListener,
    ...
    override fun createCommonViewHolder(parent: ViewGroup, viewType: Int): CommonViewHolder {
        return try {
            when (viewType) {
                TYPE_ROW -> LiveWorkoutTableViewHolder(
                    LayoutInflater
                        .from(parent.context)
    ...
    override fun onBindViewHolder(holder: CommonViewHolder, position: Int) {
        ...
        when (holder) {
            is LiveWorkoutTableViewHolder -> holder.setup(item as WorkoutModel, listener)
